...
def spike_inference(fluor, noise=0.2, gamma=0.9):
    """
...
def estimate_parameters(fluor, gamma=0.0, sigma=0.0, mode="correct"):
    """
    ...
    lags = 50
    if (gamma == 0.0) & (fluor.size > lags*2+1):
    ...
    if np.isnan(sigma):
        sys.stderr.write("Warning: sigma parameter is estimated to be zero!\n")
        sigma = 0
    return (gamma, sigma)
...
# Make a bunch of fluorescence traces with random gaussian noise and baseline
...
for x in xrange(NTRACE):
    ...
    print "tau= " + str(DELTAT/(1-gamma_est)) + " sigma = " + str(sigma_est)

    # Run spike inference
    INFERENCE[:, x], FITS[:, x] = spike_inference(
        FLUORS[x, ], noise=sigma_est, gamma=joint_gamma_est)
